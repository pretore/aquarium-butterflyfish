...
struct object {
    const struct butterflyfish_sorted_mutable_list_i
            *const sorted_mutable_list_i;
    const struct butterflyfish_sorted_settable_incremental_list_i
            *const sorted_settable_incremental_list_i;
    const struct butterflyfish_sorted_settable_reducible_list_i
            *const sorted_settable_reducible_list_i;
    const struct butterflyfish_mutable_list_i
            *const mutable_list_i;
    const struct butterflyfish_sorted_reducible_list_i
            *const sorted_reducible_list_i;
    const struct butterflyfish_sorted_incremental_list_i
            *const sorted_incremental_list_i;
    const struct butterflyfish_sorted_settable_list_i
            *const sorted_settable_list_i;
...
struct butterflyfish_sorted_list_i sorted_list_i = {
...
static int
sorted_settable_list_as_settable_list(
        struct butterflyfish_sorted_settable_list_i *const object,
        struct butterflyfish_settable_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, sorted_settable_list_i,
                              settable_list_i);
    return 0;
}

static int
sorted_settable_list_as_sorted_list(
        const struct butterflyfish_sorted_settable_list_i *const object,
...
const struct butterflyfish_sorted_settable_list_i sorted_settable_list_i = {
        .as_settable_list = sorted_settable_list_as_settable_list,
        .as_sorted_list = sorted_settable_list_as_sorted_list,
};

static int
sorted_incremental_list_as_sorted_list(
        const struct butterflyfish_sorted_incremental_list_i *const object,
        const struct butterflyfish_sorted_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, sorted_incremental_list_i,
                              sorted_list_i);
    return 0;
}

static int
sorted_incremental_list_as_incremental_list(
...
const struct butterflyfish_sorted_incremental_list_i
        sorted_incremental_list_i = {
        .as_sorted_list = sorted_incremental_list_as_sorted_list,
        .as_incremental_list = sorted_incremental_list_as_incremental_list,
};
...
mutable_list_as_settable_incremental_list(
    ...
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, mutable_list_i,
                              settable_incremental_list_i);
    return 0;
}
...
static int
sorted_settable_incremental_list_as_sorted_incremental_list(
        struct butterflyfish_sorted_settable_incremental_list_i *const object,
        struct butterflyfish_sorted_incremental_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            sorted_settable_incremental_list_i,
            sorted_incremental_list_i);
    return 0;
}

static int
sorted_settable_incremental_list_as_settable_incremental_list(
        struct butterflyfish_sorted_settable_incremental_list_i *const object,
        struct butterflyfish_settable_incremental_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
...
static int
sorted_settable_incremental_list_as_sorted_settable_list(
        struct butterflyfish_sorted_settable_incremental_list_i *const object,
    ...
    *out = butterflyfish_cast(object, struct object,
            sorted_settable_incremental_list_i,
            sorted_settable_list_i);
    return 0;
}

const struct
butterflyfish_sorted_settable_incremental_list_i
sorted_settable_incremental_list_i = {
...
static int
sorted_settable_reducible_list_as_sorted_reducible_list(
        struct butterflyfish_sorted_settable_reducible_list_i *const object,
        struct butterflyfish_sorted_reducible_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
                              sorted_settable_reducible_list_i,
                              sorted_reducible_list_i);
    return 0;
}

static int
sorted_settable_reducible_list_as_sorted_settable_list(
        struct butterflyfish_sorted_settable_reducible_list_i *const object,
        struct butterflyfish_sorted_settable_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
...
static int
sorted_settable_reducible_list_as_settable_reducible_list(
        struct butterflyfish_sorted_settable_reducible_list_i *const object,
        struct butterflyfish_settable_reducible_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
...
const struct
butterflyfish_sorted_settable_reducible_list_i
sorted_settable_reducible_list_i = {
...
static int
sorted_mutable_list_as_mutable_list(
        struct butterflyfish_sorted_mutable_list_i *const object,
        struct butterflyfish_mutable_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, sorted_mutable_list_i,
                              mutable_list_i);
    return 0;
}

static int
sorted_mutable_list_as_sorted_settable_incremental_list(
        struct butterflyfish_sorted_mutable_list_i *const object,
        struct butterflyfish_sorted_settable_incremental_list_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            sorted_mutable_list_i,
            sorted_settable_incremental_list_i);
    return 0;
}

static int
sorted_mutable_list_as_sorted_settable_reducible_list(
        struct butterflyfish_sorted_mutable_list_i *const object,
...
const struct butterflyfish_sorted_mutable_list_i sorted_mutable_list_i = {
        .as_mutable_list = sorted_mutable_list_as_mutable_list,
...
static void check_as_stream_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_stream(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_stream_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_stream((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}

static void check_as_stream(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
            .sorted_settable_reducible_list_i =
                    &sorted_settable_reducible_list_i,
            ...
            .sorted_incremental_list_i = &sorted_incremental_list_i,
            .sorted_settable_list_i = &sorted_settable_list_i,
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_stream(
            (const struct butterflyfish_sorted_mutable_list_i *) &instance,
                    &out), 0);
...
static void check_as_collection_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_collection(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_collection_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_collection((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_collection(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_collection(
            (const struct butterflyfish_sorted_mutable_list_i *) &instance,
                    &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_sorted_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_sorted_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_sorted(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted(
            (const struct butterflyfish_sorted_mutable_list_i *) &instance,
                    &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_settable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_settable(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_settable_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_settable((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_settable(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_settable(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_addable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_addable(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_addable_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_addable((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_addable(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_addable(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_list(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_list((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_list(
            (const struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_removable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_removable(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_removable_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_removable((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_removable(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_removable(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_sorted_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_list(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_sorted_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_list((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_sorted_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_list(
            (const struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_settable_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_settable_list(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_settable_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_settable_list((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_settable_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_settable_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_incremental_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_incremental_list(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_incremental_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_incremental_list((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_incremental_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_incremental_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_reducible_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_reducible_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_reducible_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_reducible_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
check_as_settable_incremental_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_settable_incremental_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_settable_incremental_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_settable_incremental_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
check_as_settable_reducible_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_settable_reducible_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_settable_reducible_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_settable_reducible_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void
check_as_sorted_settable_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_settable_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_sorted_settable_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_settable_list(
                    (void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
...
static void check_as_sorted_settable_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, sorted_settable_list_i);
    struct butterflyfish_sorted_settable_list_i *out;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_settable_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void
check_as_sorted_incremental_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_incremental_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void
check_as_sorted_incremental_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_incremental_list(
                    (void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
...
static void check_as_sorted_incremental_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, sorted_incremental_list_i);
    struct butterflyfish_sorted_incremental_list_i *out;
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_incremental_list(
                    (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
...
static void
check_as_sorted_reducible_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_reducible_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void
check_as_sorted_reducible_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_reducible_list(
                    (void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
...
static void check_as_sorted_reducible_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, sorted_reducible_list_i);
...
static void
check_as_sorted_settable_incremental_list_error_on_object_is_null(
        void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_settable_incremental_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void
check_as_sorted_settable_incremental_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_settable_incremental_list(
                    (void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
...
static void check_as_sorted_settable_incremental_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, sorted_settable_incremental_list_i);
    struct butterflyfish_sorted_settable_incremental_list_i *out;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_settable_incremental_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void
check_as_sorted_settable_reducible_list_error_on_object_is_null(
        void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_settable_reducible_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void
check_as_sorted_settable_reducible_list_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_settable_reducible_list(
                    (void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
...
static void check_as_sorted_settable_reducible_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, sorted_settable_reducible_list_i);
    struct butterflyfish_sorted_settable_reducible_list_i *out;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_settable_reducible_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_mutable_list_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_mutable_list(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_mutable_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_mutable_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
                    &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_mutable_list_as_reducible_list(void **state) {
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_mutable_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
                    &as), 0);
    struct butterflyfish_reducible_list_i *out;
    assert_int_equal(butterflyfish_mutable_list_i_as_reducible_list(as, &out),
                     0);
    assert_ptr_equal(out, check);
}
...
static void
check_sorted_settable_reducible_list_as_settable_reducible_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, settable_reducible_list_i);
    struct butterflyfish_sorted_settable_reducible_list_i *as;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_settable_reducible_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &as), 0);
    struct butterflyfish_settable_reducible_list_i *out;
    assert_int_equal(butterflyfish_sorted_settable_reducible_list_i_as_settable_reducible_list(
            as, &out), 0);
    assert_ptr_equal(out, check);
}

static void
check_sorted_settable_reducible_list_as_sorted_settable_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_settable_reducible_list(
            ...
            &as), 0);
    struct butterflyfish_sorted_settable_list_i *out;
    assert_int_equal(butterflyfish_sorted_settable_reducible_list_i_as_sorted_settable_list(
            as, &out), 0);
    assert_ptr_equal(out, check);
}

static void
check_sorted_settable_incremental_list_as_settable_incremental_list(
        void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, settable_incremental_list_i);
    struct butterflyfish_sorted_settable_incremental_list_i *as;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_settable_incremental_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &as), 0);
    struct butterflyfish_settable_incremental_list_i *out;
    assert_int_equal(butterflyfish_sorted_settable_incremental_list_i_as_settable_incremental_list(
            as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_sorted_reducible_list_as_sorted_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, sorted_list_i);
    struct butterflyfish_sorted_reducible_list_i *as;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_as_sorted_reducible_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &as), 0);
    const struct butterflyfish_sorted_list_i *out;
...
static void check_sorted_incremental_list_as_sorted_list(void **state) {
    const struct object instance = {
            .sorted_mutable_list_i = &sorted_mutable_list_i,
            .sorted_settable_incremental_list_i =
                    &sorted_settable_incremental_list_i,
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, sorted_list_i);
    struct butterflyfish_sorted_incremental_list_i *as;
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_as_sorted_incremental_list(
            (struct butterflyfish_sorted_mutable_list_i *) &instance,
            &as), 0);
    const struct butterflyfish_sorted_list_i *out;
    assert_int_equal(butterflyfish_sorted_incremental_list_i_as_sorted_list(
            as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_count_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_count(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_count_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_count((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_count(void **state) {
    ...
    uintmax_t out;
    assert_int_equal(butterflyfish_sorted_mutable_list_i_count(
            (const struct butterflyfish_sorted_mutable_list_i *) &instance,
            &out), 0);
...
static void check_first_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_first(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_first_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_first((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_first_error_on_list_is_empty(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_first(
                    (const struct butterflyfish_sorted_mutable_list_i *)
                            &instance, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_LIST_IS_EMPTY);
}

static void check_last_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_last(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_last_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_last((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_last_error_on_list_is_empty(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_last(
                    (const struct butterflyfish_sorted_mutable_list_i *)
                            &instance, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_LIST_IS_EMPTY);
}
...
static void check_next_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_next(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_next_error_on_item_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_next(
                    (void *) 1, NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_ITEM_IS_NULL);
}
...
static void check_next_error_on_end_of_sequence(void **state) {
    ...
    assert_int_equal(
            ...
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_END_OF_SEQUENCE);
}

static void check_prev_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_prev(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_get_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_get(NULL, 0, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_get_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_get((void *) 1, 0, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OUT_IS_NULL);
}
...
static void check_get_error_on_index_out_of_bounds(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_get(
                    (const struct butterflyfish_sorted_mutable_list_i *)
                            &instance, 0, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_INDEX_IS_OUT_OF_BOUNDS);
}

static void check_at_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_at(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_at_error_on_item_not_found(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_at(
                    (const struct butterflyfish_sorted_mutable_list_i *)
                            &instance, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_ITEM_NOT_FOUND);
}

static void check_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_set(NULL, 0, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_set_error_on_value_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_set((void *) 1, 0, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_VALUE_IS_NULL);
}
...
static void check_set_error_on_index_out_of_bounds(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_set(
                    (struct butterflyfish_sorted_mutable_list_i *) &instance,
                    0,
                    (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_INDEX_IS_OUT_OF_BOUNDS);
...
static void check_set_error_on_memory_allocation_failed(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_set(
                    ...
                    0,
                    (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_MEMORY_ALLOCATION_FAILED);
}

static void check_set_item_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_set_item(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_set_item_error_on_value_is_invalid(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_set_item(
                    (struct butterflyfish_sorted_mutable_list_i *) &instance,
                    (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_VALUE_IS_INVALID);
}

static void check_ceiling_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_ceiling(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_ceiling_error_on_value_not_found(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_ceiling(
                    (const struct butterflyfish_sorted_mutable_list_i *)
                            &instance, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_VALUE_NOT_FOUND);
}
...
static void check_floor_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_floor(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_higher_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_higher(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_lower_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_lower(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_add_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_add(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_add_error_on_value_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_add((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_VALUE_IS_NULL);
}
...
static void check_add_error_on_memory_allocation_failed(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_add(
                    (struct butterflyfish_sorted_mutable_list_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_MEMORY_ALLOCATION_FAILED);
}
...
static void check_add_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_add_all(NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}

static void check_add_all_error_on_other_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_add_all((void *) 1, NULL),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OTHER_IS_NULL);
}
...
static void check_add_all_error_on_memory_allocation_failed(void **state) {
    ...
    will_return(addable_add_all,
                BUTTERFLYFISH_ADDABLE_I_ERROR_MEMORY_ALLOCATION_FAILED);
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_add_all(
                    (struct butterflyfish_sorted_mutable_list_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_MEMORY_ALLOCATION_FAILED);
}
...
static void check_remove_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_remove(NULL, 0),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_remove(void **state) {
    ...
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_remove(
                    (struct butterflyfish_sorted_mutable_list_i *) &instance, 0),
            0);
}
...
static void check_remove_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_remove_all(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_remove_item_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_remove_item(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_remove_all_items_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_sorted_mutable_list_i_remove_all_items(
                    NULL, (void *) 1),
            BUTTERFLYFISH_SORTED_MUTABLE_LIST_I_ERROR_OBJECT_IS_NULL);
}
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_as_sorted_settable_list_error_on_object_is_null),
            cmocka_unit_test(check_as_sorted_settable_list_error_on_out_is_null),
            cmocka_unit_test(check_as_sorted_settable_list),
            cmocka_unit_test(check_as_sorted_incremental_list_error_on_object_is_null),
            cmocka_unit_test(check_as_sorted_incremental_list_error_on_out_is_null),
            cmocka_unit_test(check_as_sorted_incremental_list),
            cmocka_unit_test(check_as_sorted_reducible_list_error_on_object_is_null),
            cmocka_unit_test(check_as_sorted_reducible_list_error_on_out_is_null),
            cmocka_unit_test(check_as_sorted_reducible_list),
            cmocka_unit_test(check_as_sorted_settable_incremental_list_error_on_object_is_null),
            cmocka_unit_test(check_as_sorted_settable_incremental_list_error_on_out_is_null),
            cmocka_unit_test(check_as_sorted_settable_incremental_list),
            cmocka_unit_test(check_as_sorted_settable_reducible_list_error_on_object_is_null),
            cmocka_unit_test(check_as_sorted_settable_reducible_list_error_on_out_is_null),
            cmocka_unit_test(check_as_sorted_settable_reducible_list),
            ...
            cmocka_unit_test(check_sorted_settable_reducible_list_as_settable_reducible_list),
            cmocka_unit_test(check_sorted_settable_reducible_list_as_sorted_settable_list),
            cmocka_unit_test(check_sorted_settable_incremental_list_as_settable_incremental_list),


            cmocka_unit_test(check_settable_reducible_list_as_settable_list),

            cmocka_unit_test(check_sorted_reducible_list_as_sorted_list),
            cmocka_unit_test(check_sorted_incremental_list_as_sorted_list),
