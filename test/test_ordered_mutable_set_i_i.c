...
struct object {
    const struct butterflyfish_ordered_mutable_set_i_i *const
            ordered_mutable_set_i_i;
    const struct butterflyfish_ordered_settable_incremental_set_i_i *const
            ordered_settable_incremental_set_i_i;
    const struct butterflyfish_ordered_settable_reducible_set_i_i *const
            ordered_settable_reducible_set_i_i;
    const struct butterflyfish_ordered_reducible_set_i_i *const
            ordered_reducible_set_i_i;
    const struct butterflyfish_ordered_incremental_set_i_i *const
            ordered_incremental_set_i_i;
    const struct butterflyfish_ordered_settable_set_i_i *const
            ordered_settable_set_i_i;
    const struct butterflyfish_ordered_set_i_i *const ordered_set_i_i;
    const struct butterflyfish_mutable_set_i_i *const mutable_set_i_i;
    const struct butterflyfish_settable_incremental_set_i_i *const
            settable_incremental_set_i_i;
    const struct butterflyfish_settable_reducible_set_i_i *const
            settable_reducible_set_i_i;
    const struct butterflyfish_reducible_set_i_i *const reducible_set_i_i;
    const struct butterflyfish_incremental_set_i_i *const incremental_set_i_i;
...
static int
settable_set_as_settable(
        struct butterflyfish_settable_set_i_i *const object,
        struct butterflyfish_settable_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, settable_set_i_i, settable_i_i);
    return 0;
}

static int settable_set_set(struct butterflyfish_settable_set_i_i *const object,
                         const struct butterflyfish_map_i_i_entry *const item,
                         const struct butterflyfish_map_i_i_entry *const value) {
    function_called();
    assert_non_null(object);
...
static int
ordered_set_as_ordered(const struct butterflyfish_ordered_set_i_i *const object,
                       const struct butterflyfish_ordered_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, ordered_set_i_i, ordered_i_i);
    return 0;
}

static int
ordered_set_as_set(const struct butterflyfish_ordered_set_i_i *const object,
                   const struct butterflyfish_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, ordered_set_i_i, set_i_i);
    return 0;
}

const struct butterflyfish_ordered_set_i_i ordered_set_i_i = {
...
ordered_settable_set_as_ordered_set(
    ...
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, ordered_settable_set_i_i,
                              ordered_set_i_i);
    return 0;
}
...
const struct butterflyfish_ordered_settable_set_i_i ordered_settable_set_i_i = {
...
incremental_set_as_addable(
        struct butterflyfish_incremental_set_i_i *const object,
        struct butterflyfish_addable_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, incremental_set_i_i,
            addable_i_i);
    return 0;
}
...
incremental_set_as_set(
        const struct butterflyfish_incremental_set_i_i *const object,
        const struct butterflyfish_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, incremental_set_i_i, set_i_i);
    return 0;
}
...
static int insertable_as_ordered(
...
static int insertable_insert(
        struct butterflyfish_insertable_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const item,
        const struct butterflyfish_map_i_i_entry *const value) {
    function_called();
    assert_non_null(object);
...
ordered_incremental_set_as_ordered_set(
        const struct butterflyfish_ordered_incremental_set_i_i *const object,
        const struct butterflyfish_ordered_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, ordered_incremental_set_i_i,
                              ordered_set_i_i);
    return 0;
}
...
ordered_incremental_set_as_incremental_set(
        struct butterflyfish_ordered_incremental_set_i_i *const object,
        struct butterflyfish_incremental_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            ordered_incremental_set_i_i, incremental_set_i_i);
    return 0;
}
...
ordered_incremental_set_as_insertable(
    ...
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            ordered_incremental_set_i_i, insertable_i_i);
    return 0;
}

const struct
butterflyfish_ordered_incremental_set_i_i ordered_incremental_set_i_i = {
...
reducible_set_as_set(
        const struct butterflyfish_reducible_set_i_i *const object,
        const struct butterflyfish_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
...
ordered_reducible_set_as_ordered_set(
    ...
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, ordered_reducible_set_i_i,
                              ordered_set_i_i);
    return 0;
}
...
const struct
butterflyfish_ordered_reducible_set_i_i ordered_reducible_set_i_i = {
...
settable_incremental_set_as_settable_set(
        struct butterflyfish_settable_incremental_set_i_i *const object,
        struct butterflyfish_settable_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            settable_incremental_set_i_i, settable_set_i_i);
    return 0;
}
...
settable_incremental_set_as_incremental_set(
    ...
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            settable_incremental_set_i_i, incremental_set_i_i);
    return 0;
}

const struct
butterflyfish_settable_incremental_set_i_i settable_incremental_set_i_i = {
...
settable_reducible_set_as_settable_set(
    ...
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            settable_reducible_set_i_i, settable_set_i_i);
    return 0;
}
...
settable_reducible_set_as_reducible_set(
    ...
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
            settable_reducible_set_i_i, reducible_set_i_i);
    return 0;
}
...
mutable_set_as_settable_incremental_set(
        struct butterflyfish_mutable_set_i_i *const object,
        struct butterflyfish_settable_incremental_set_i_i **const out) {
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, mutable_set_i_i,
                              settable_incremental_set_i_i);
    return 0;
}
...
const struct butterflyfish_mutable_set_i_i mutable_set_i_i = {
...
ordered_settable_incremental_set_as_settable_incremental_set(
        struct butterflyfish_ordered_settable_incremental_set_i_i *const object,
        struct butterflyfish_settable_incremental_set_i_i **const out) {
        assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
...
const struct butterflyfish_ordered_settable_incremental_set_i_i
ordered_settable_incremental_set_i_i = {
...
ordered_settable_reducible_set_as_settable_reducible_set(
        struct butterflyfish_ordered_settable_reducible_set_i_i *const object,
        struct butterflyfish_settable_reducible_set_i_i **const out) {
        assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object,
...
const struct butterflyfish_ordered_settable_reducible_set_i_i
ordered_settable_reducible_set_i_i = {
...
static int
mutable_ordered_set_as_ordered_settable_incremental_set(
    ...
    assert_non_null(object);
    assert_non_null(out);
    *out = butterflyfish_cast(object, struct object, ordered_mutable_set_i_i,
                              ordered_settable_incremental_set_i_i);
    return 0;
}

static int
mutable_ordered_set_as_ordered_settable_reducible_set(
        struct butterflyfish_ordered_mutable_set_i_i *const object,
...
static int
mutable_ordered_set_as_mutable_set(
        struct butterflyfish_ordered_mutable_set_i_i *const object,
...
const struct butterflyfish_ordered_mutable_set_i_i ordered_mutable_set_i_i = {
        .as_ordered_settable_incremental_set =
                mutable_ordered_set_as_ordered_settable_incremental_set,
        .as_ordered_settable_reducible_set =
                mutable_ordered_set_as_ordered_settable_reducible_set,
        .as_mutable_set = mutable_ordered_set_as_mutable_set,
};

static void check_as_stream_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_stream(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_stream_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_stream(
                    (void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}

static void check_as_stream(void **state) {
    const struct object instance = {
            .ordered_mutable_set_i_i = &ordered_mutable_set_i_i,
            .ordered_settable_incremental_set_i_i =
                    &ordered_settable_incremental_set_i_i,
            .ordered_settable_reducible_set_i_i =
                    &ordered_settable_reducible_set_i_i,
            .ordered_reducible_set_i_i = &ordered_reducible_set_i_i,
            .ordered_incremental_set_i_i = &ordered_incremental_set_i_i,
            .ordered_settable_set_i_i = &ordered_settable_set_i_i,
            .ordered_set_i_i = &ordered_set_i_i,
            .mutable_set_i_i = &mutable_set_i_i,
            .settable_incremental_set_i_i = &settable_incremental_set_i_i,
            .settable_reducible_set_i_i = &settable_reducible_set_i_i,
            .reducible_set_i_i = &reducible_set_i_i,
            .incremental_set_i_i = &incremental_set_i_i,
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_stream(
            (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    &out), 0);
...
static void check_as_collection_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_collection(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_collection(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_collection(
            (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_addable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_addable(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_addable_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_addable((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_addable(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_addable(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_removable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_removable(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_removable_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_removable((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_removable(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_removable(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_settable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_settable_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_settable(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_settable(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_ordered_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_ordered_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_ordered(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_ordered(
            (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_set(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_set_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_set((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_set(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_set(
            (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_settable_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable_set(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_as_settable_set_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable_set((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_as_settable_set(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_settable_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_incremental_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_incremental_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_incremental_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, incremental_set_i_i);
    struct butterflyfish_incremental_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_incremental_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_reducible_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_reducible_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_reducible_set(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_reducible_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
check_as_settable_incremental_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable_incremental_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_settable_incremental_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, settable_incremental_set_i_i);
    struct butterflyfish_settable_incremental_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_settable_incremental_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
check_as_settable_reducible_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable_reducible_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_settable_reducible_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, settable_reducible_set_i_i);
    struct butterflyfish_settable_reducible_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_settable_reducible_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_mutable_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_mutable_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_mutable_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, mutable_set_i_i);
    struct butterflyfish_mutable_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_mutable_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_ordered_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_ordered_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_set_i_i);
    const struct butterflyfish_ordered_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_ordered_set(
            (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_insertable_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_insertable(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_insertable(void **state) {
    ...
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_insertable(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_as_ordered_settable_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_ordered_settable_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_settable_set_i_i);
    struct butterflyfish_ordered_settable_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
    assert_ptr_equal(out, check);
}
...
check_as_ordered_incremental_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_incremental_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_ordered_incremental_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_incremental_set_i_i);
    struct butterflyfish_ordered_incremental_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_incremental_set(
            (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
            &out), 0);
...
check_as_ordered_reducible_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_reducible_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_ordered_reducible_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_reducible_set_i_i);
    struct butterflyfish_ordered_reducible_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_reducible_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &out), 0);
    assert_ptr_equal(out, check);
}

static void
check_as_ordered_settable_incremental_set_error_on_object_is_null(
        void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_incremental_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_ordered_settable_incremental_set(void **state) {
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, ordered_settable_incremental_set_i_i);
    struct butterflyfish_ordered_settable_incremental_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_incremental_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &out), 0);
...
check_as_ordered_settable_reducible_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_reducible_set(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_as_ordered_settable_reducible_set(void **state) {
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, ordered_settable_reducible_set_i_i);
    struct butterflyfish_ordered_settable_reducible_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_reducible_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &out), 0);
...
check_ordered_settable_reducible_set_as_ordered_settable_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_settable_set_i_i);
    struct butterflyfish_ordered_settable_reducible_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_reducible_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    struct butterflyfish_ordered_settable_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_settable_reducible_set_i_i_as_ordered_settable_set(
            as, &out), 0);
    assert_ptr_equal(out, check);
}
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_reducible_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    struct butterflyfish_settable_reducible_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_settable_reducible_set_i_i_as_settable_reducible_set(
            as, &out), 0);
    assert_ptr_equal(out, check);
}

static void
check_ordered_settable_incremental_set_as_settable_incremental_set(void **state) {
    const struct object instance = {
            .ordered_mutable_set_i_i = &ordered_mutable_set_i_i,
            .ordered_settable_incremental_set_i_i =
                    &ordered_settable_incremental_set_i_i,
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, settable_incremental_set_i_i);
    struct butterflyfish_ordered_settable_incremental_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_incremental_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    struct butterflyfish_settable_incremental_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_settable_incremental_set_i_i_as_settable_incremental_set(
            as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_settable_set_as_ordered_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_set_i_i);
    struct butterflyfish_ordered_settable_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_settable_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    const struct butterflyfish_ordered_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_settable_set_i_i_as_ordered_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_settable_set_as_settable_set(void **state) {
    ...
    struct butterflyfish_settable_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_settable_set_i_i_as_settable_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_incremental_set_as_insertable(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, insertable_i_i);
    struct butterflyfish_ordered_incremental_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_incremental_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    struct butterflyfish_insertable_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_incremental_set_i_i_as_insertable(as, &out),
            0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_incremental_set_as_ordered_set(void **state) {
    ...
    const struct butterflyfish_ordered_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_incremental_set_i_i_as_ordered_set(as, &out),
            0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_incremental_set_as_incremental_set(void **state) {
    ...
    struct butterflyfish_incremental_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_incremental_set_i_i_as_incremental_set(
                    as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_reducible_set_as_ordered_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, ordered_set_i_i);
    struct butterflyfish_ordered_reducible_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_reducible_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    const struct butterflyfish_ordered_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_reducible_set_i_i_as_ordered_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_ordered_reducible_set_as_reducible_set(void **state) {
    ...
    struct butterflyfish_reducible_set_i_i *out;
    assert_int_equal(
            butterflyfish_ordered_reducible_set_i_i_as_reducible_set(
                    as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_mutable_set_as_settable_set(void **state) {
    ...
    const void *check = (char *) &instance
                        + offsetof(struct object, settable_set_i_i);
    struct butterflyfish_mutable_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_mutable_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    struct butterflyfish_settable_set_i_i *out;
    assert_int_equal(butterflyfish_mutable_set_i_i_as_settable_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_mutable_set_as_incremental_set(void **state) {
    ...
    struct butterflyfish_incremental_set_i_i *out;
    assert_int_equal(
            butterflyfish_mutable_set_i_i_as_incremental_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_mutable_set_as_reducible_set(void **state) {
    ...
    struct butterflyfish_reducible_set_i_i *out;
    assert_int_equal(
            butterflyfish_mutable_set_i_i_as_reducible_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_insertable_as_ordered(void **state) {
    ...
    const void *check = (char *) &instance + offsetof(struct object, ordered_i_i);
...
static void check_ordered_set_as_set(void **state) {
    ...
    const void *check = (char *) &instance + offsetof(struct object, set_i_i);
    const struct butterflyfish_ordered_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_ordered_set(
                    (const struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    const struct butterflyfish_set_i_i *out;
    assert_int_equal(butterflyfish_ordered_set_i_i_as_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_settable_set_as_set(void **state) {
    ...
    const void *check = (char *) &instance + offsetof(struct object, set_i_i);
    struct butterflyfish_settable_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    const struct butterflyfish_set_i_i *out;
    assert_int_equal(butterflyfish_settable_set_i_i_as_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_settable_reducible_set_as_settable_set(void **state) {
    ...
    const void *check = (char *) &instance
            + offsetof(struct object, settable_set_i_i);
    struct butterflyfish_settable_reducible_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_settable_reducible_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
...
static void check_incremental_set_as_set(void **state) {
    ...
    const void *check = (char *) &instance + offsetof(struct object, set_i_i);
    struct butterflyfish_incremental_set_i_i *as;
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_as_incremental_set(
                    (struct butterflyfish_ordered_mutable_set_i_i *)
                            &instance, &as), 0);
    const struct butterflyfish_set_i_i *out;
    assert_int_equal(butterflyfish_incremental_set_i_i_as_set(as, &out), 0);
    assert_ptr_equal(out, check);
}
...
static void check_count_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_count(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_count_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_count((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_count(void **state) {
    ...
    uintmax_t out;
    assert_int_equal(butterflyfish_ordered_mutable_set_i_i_count(
            (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    &out), 0);
...
static void check_first_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_first(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_first_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_first((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_first_error_on_set_is_empty(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_first(
                    (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_SET_IS_EMPTY);
}

static void check_last_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_last(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_last_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_last((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OUT_IS_NULL);
}
...
static void check_last_error_on_set_is_empty(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_last(
                    (const struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_SET_IS_EMPTY);
}
...
static void check_next_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_next(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_next_error_on_item_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_next(
                    (void *) 1, NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_ITEM_IS_NULL);
}
...
static void check_next_error_on_end_of_sequence(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_next(
                    ...
                    (void *) 1,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_END_OF_SEQUENCE);
}

static void check_prev_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_prev(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_contains_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_contains(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_contains_error_on_value_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_contains(
                    (void *) 1, NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_IS_NULL);
}
...
static void check_contains_error_on_memory_allocation_failed(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_contains(
                    ...
                    (void *) 1,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
}

static void check_contains_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_contains_all(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_contains_all_error_on_other_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_contains_all(
                    (void *) 1, NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OTHER_IS_NULL);
}
...
static void check_get_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_get(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_get_error_on_value_not_found(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_get(
                    ...
                    (void *) 1,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_NOT_FOUND);
}
...
static void check_set_item_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_set_item(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_set_item_error_on_value_is_invalid(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_set_item(
                    ...
                    (void *) 1,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_IS_INVALID);
}
...
static void check_set_item_error_on_value_already_exists(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_set_item(
                    ...
                    (void *) 1,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_ALREADY_EXISTS);
}
...
static void check_set_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_set(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_add_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_add(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_add_error_on_value_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_add((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_IS_NULL);
}
...
static void check_add_error_on_value_already_exits(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_add(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_ALREADY_EXISTS);
}
...
static void check_add_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_add_all(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_add_all_error_on_other_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_add_all((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OTHER_IS_NULL);
}
...
static void check_add_all_error_on_memory_allocation_failed(void **state) {
    ...
    will_return(addable_add_all,
                BUTTERFLYFISH_ADDABLE_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_add_all(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
}
...
static void check_insert_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_insert(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_insert_error_on_value_already_exits(void **state) {
    const struct object instance = {
            .ordered_mutable_set_i_i = &ordered_mutable_set_i_i,
            .ordered_settable_incremental_set_i_i =
                    &ordered_settable_incremental_set_i_i,
    ...
    expect_function_call(insertable_insert);
    will_return(insertable_insert,
                BUTTERFLYFISH_INSERTABLE_I_I_ERROR_VALUE_ALREADY_EXISTS);
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_insert(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1,
...
static void check_insert_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_insert_all(
                    NULL, (void *) 1, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_remove_item_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_item(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_remove_item_error_on_item_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_item((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_ITEM_IS_NULL);
}
...
static void check_remove_item(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_item(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1), 0);
}
...
static void check_remove_all_items_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_all_items(
                    NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}
...
static void check_remove_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_remove_error_on_value_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_IS_NULL);
}
...
static void check_remove_error_on_value_not_found(void **state) {
    ...
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_VALUE_NOT_FOUND);
}
...
static void check_remove_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_all(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_remove_all_error_on_other_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_all((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OTHER_IS_NULL);
}
...
static void check_remove_all_error_on_memory_allocation_failed(void **state) {
    ...
    will_return(reducible_set_remove_all,
                BUTTERFLYFISH_REDUCIBLE_SET_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_remove_all(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
}
...
static void check_retain_all_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_retain_all(NULL, (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OBJECT_IS_NULL);
}

static void check_retain_all_error_on_other_is_null(void **state) {
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_retain_all((void *) 1, NULL),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_OTHER_IS_NULL);
}
...
static void check_retain_all_error_on_memory_allocation_failed(void **state) {
    ...
    will_return(reducible_set_retain_all,
                BUTTERFLYFISH_REDUCIBLE_SET_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
    assert_int_equal(
            butterflyfish_ordered_mutable_set_i_i_retain_all(
                    (struct butterflyfish_ordered_mutable_set_i_i *) &instance,
                    (void *) 1),
            BUTTERFLYFISH_ORDERED_MUTABLE_SET_I_I_ERROR_MEMORY_ALLOCATION_FAILED);
}
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_insert_error_on_value_already_exits),
