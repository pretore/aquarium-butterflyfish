...
static void check_as_collection(void **state) {
    ...
    assert_int_equal(butterflyfish_queue_wr_as_collection(
            &queue, &collection), 0);
...
static void check_as_reducible_queue_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_queue_wr_as_reducible_queue(NULL, (void *) 1),
            BUTTERFLYFISH_QUEUE_WR_ERROR_OBJECT_IS_NULL);
}

static void check_as_reducible_queue_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_queue_wr_as_reducible_queue((void *) 1, NULL),
            BUTTERFLYFISH_QUEUE_WR_ERROR_OUT_IS_NULL);
}

static void check_as_reducible_queue(void **state) {
    struct butterflyfish_queue_wr queue = {};
    struct butterflyfish_reducible_queue_wr *reducible_queue;
    assert_int_equal(butterflyfish_queue_wr_as_reducible_queue(
...
static void check_count(void **state) {
    ...
    const struct butterflyfish_queue_wr queue_wr = {
            .reducible_queue_wr.collection_wr.count = count
    };
    struct object {
        const struct butterflyfish_queue_wr *queue_wr;
    };
...
static void check_add_error_on_value_is_invalid(void **state) {
    ...
    expect_function_call(add_emit_error);
    will_return(add_emit_error,
                BUTTERFLYFISH_QUEUE_WR_ERROR_VALUE_IS_INVALID);
    assert_int_equal(
            butterflyfish_queue_wr_add(
                    (struct butterflyfish_queue_wr *) &instance,
                    (void *) 1),
