...
static void check_as_stream_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_collection_s_as_stream(NULL, (void *) 1),
            BUTTERFLYFISH_COLLECTION_S_ERROR_OBJECT_IS_NULL);
}

static void check_as_stream_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_collection_s_as_stream((void *) 1, NULL),
            BUTTERFLYFISH_COLLECTION_S_ERROR_OUT_IS_NULL);
}

static void check_as_stream(void **state) {
    struct butterflyfish_collection_s collection = {};
    const struct butterflyfish_stream_s *stream;
    assert_int_equal(butterflyfish_collection_s_as_stream(
            &collection, &stream), 0);
