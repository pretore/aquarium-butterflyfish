...
static void check_as_stream_error_on_object_is_null(void **state) {
    assert_int_equal(
            butterflyfish_collection_p_p_as_stream(NULL, (void *) 1),
            BUTTERFLYFISH_COLLECTION_P_P_ERROR_OBJECT_IS_NULL);
}

static void check_as_stream_error_on_out_is_null(void **state) {
    assert_int_equal(
            butterflyfish_collection_p_p_as_stream((void *) 1, NULL),
            BUTTERFLYFISH_COLLECTION_P_P_ERROR_OUT_IS_NULL);
}

static void check_as_stream(void **state) {
    struct butterflyfish_collection_p_p collection = {};
    const struct butterflyfish_stream_p_p *stream;
    assert_int_equal(butterflyfish_collection_p_p_as_stream(
            &collection, &stream), 0);
