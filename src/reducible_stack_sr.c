...
#define INVOKABLE   (*(struct butterflyfish_reducible_stack_sr **) object)

#pragma mark stream_sr -

int butterflyfish_reducible_stack_sr_first(
        const struct butterflyfish_reducible_stack_sr *const object,
        struct triggerfish_strong **const out) {
    if (!object) {
        return BUTTERFLYFISH_REDUCIBLE_STACK_SR_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_REDUCIBLE_STACK_SR_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_reducible_stack_sr_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_REDUCIBLE_STACK_SR_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_REDUCIBLE_STACK_SR_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_reducible_stack_sr_pop(
        struct butterflyfish_reducible_stack_sr *const object,
        struct triggerfish_strong **const out) {
    if (!object) {
        return BUTTERFLYFISH_REDUCIBLE_STACK_SR_ERROR_OBJECT_IS_NULL;
    }
