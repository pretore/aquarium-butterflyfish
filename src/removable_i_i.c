...
#define INVOKE(x)               (*(struct butterflyfish_removable_i_i **) x)
...
static inline int
as_stream(const struct butterflyfish_removable_i_i *const object,
...
int butterflyfish_removable_i_i_first(
    ...
    if (!out) {
        return BUTTERFLYFISH_REMOVABLE_I_I_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_removable_i_i_remove_all_items(
    ...
    if (!other) {
        return BUTTERFLYFISH_REMOVABLE_I_I_ERROR_OTHER_IS_NULL;
    }
