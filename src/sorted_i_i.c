...
#define INVOKE(x)               (*(struct butterflyfish_sorted_i_i **) x)
...
static inline int
as_stream(const struct butterflyfish_sorted_i_i *const object,
...
int butterflyfish_sorted_i_i_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_SORTED_I_I_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_SORTED_I_I_ERROR_OUT_IS_NULL;
    }
