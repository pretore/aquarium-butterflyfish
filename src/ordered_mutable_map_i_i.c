#include <stdlib.h>
#include <assert.h>
#include <seagrass.h>
#include <butterflyfish.h>

#ifdef TEST
#include <test/cmocka.h>
#endif

#define INVOKE(x) \
    (*(struct butterflyfish_ordered_mutable_map_i_i **) x)
#define INVOKE_ORDERED_SETTABLE_INCREMENTAL_MAP(x) \
    (*(struct butterflyfish_ordered_settable_incremental_map_i_i **) x)
#define INVOKE_ORDERED_SETTABLE_REDUCIBLE_MAP(x) \
    (*(struct butterflyfish_ordered_settable_reducible_map_i_i **) x)
#define INVOKE_MUTABLE_MAP(x) \
    (*(struct butterflyfish_mutable_map_i_i **) x)
#define INVOKE_SETTABLE_INCREMENTAL_MAP(x) \
    (*(struct butterflyfish_settable_incremental_map_i_i **) x)
#define INVOKE_SETTABLE_REDUCIBLE_MAP(x) \
    (*(struct butterflyfish_settable_reducible_map_i_i **) x)
#define INVOKE_ORDERED_SETTABLE_MAP(x) \
    (*(struct butterflyfish_ordered_settable_map_i_i **) x)
#define INVOKE_ORDERED_INCREMENTAL_MAP(x) \
    (*(struct butterflyfish_ordered_incremental_map_i_i **) x)
#define INVOKE_ORDERED_REDUCIBLE_MAP(x) \
    (*(struct butterflyfish_ordered_reducible_map_i_i **) x)
#define INVOKE_ORDERED_MAP(x) \
    (*(struct butterflyfish_ordered_map_i_i **) x)
#define INVOKE_SETTABLE_MAP(x) \
    (*(struct butterflyfish_settable_map_i_i **) x)
#define INVOKE_INCREMENTAL_MAP(x) \
    (*(struct butterflyfish_incremental_map_i_i **) x)
#define INVOKE_REDUCIBLE_MAP(x) \
    (*(struct butterflyfish_reducible_map_i_i **) x)
#define INVOKE_MAP(x)           (*(struct butterflyfish_map_i_i **) x)
#define INVOKE_INSERTABLE(x)    (*(struct butterflyfish_insertable_i_i **) x)
#define INVOKE_ORDERED(x)       (*(struct butterflyfish_ordered_i_i **) x)
#define INVOKE_ADDABLE(x)       (*(struct butterflyfish_addable_i_i **) x)
#define INVOKE_REMOVABLE(x)     (*(struct butterflyfish_removable_i_i **) x)
#define INVOKE_SETTABLE(x)      (*(struct butterflyfish_settable_i_i **) x)
#define INVOKE_SET(x)           (*(struct butterflyfish_set_i_i **) x)
#define INVOKE_COLLECTION(x)    (*(struct butterflyfish_collection_i_i **) x)
#define INVOKE_STREAM(x)        (*(struct butterflyfish_stream_i_i **) x)

static inline int
as_ordered_settable_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_incremental_map_i_i
                **const out) {
    assert(object);
    assert(out);
    return INVOKE(object)->as_ordered_settable_incremental_map(object, out);
}

static inline int
as_ordered_settable_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_reducible_map_i_i **const out) {
    assert(object);
    assert(out);
    return INVOKE(object)->as_ordered_settable_reducible_map(object, out);
}

static inline int
as_mutable_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_mutable_map_i_i **const out) {
    assert(object);
    assert(out);
    return INVOKE(object)->as_mutable_map(object, out);
}

static inline int
as_ordered_settable_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_incremental_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_incremental_map(object, &map));
    return INVOKE_ORDERED_SETTABLE_INCREMENTAL_MAP(map)
            ->as_ordered_settable_map(map, out);
}

static inline int
as_ordered_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_incremental_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_incremental_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_incremental_map(object, &map));
    return INVOKE_ORDERED_SETTABLE_INCREMENTAL_MAP(map)
            ->as_ordered_incremental_map(map, out);
}

static inline int
as_ordered_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_reducible_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_reducible_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_reducible_map(object, &map));
    return INVOKE_ORDERED_SETTABLE_REDUCIBLE_MAP(map)
            ->as_ordered_reducible_map(map, out);
}

static inline int
as_settable_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_settable_incremental_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_incremental_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_incremental_map(object, &map));
    return INVOKE_ORDERED_SETTABLE_INCREMENTAL_MAP(map)
            ->as_settable_incremental_map(map, out);
}

static inline int
as_settable_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_settable_reducible_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_reducible_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_reducible_map(object, &map));
    return INVOKE_ORDERED_SETTABLE_REDUCIBLE_MAP(map)
        ->as_settable_reducible_map(map, out);
}

static inline int
as_ordered_map(const struct butterflyfish_ordered_mutable_map_i_i *const object,
               const struct butterflyfish_ordered_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_map(
            (void *) object, &map));
    return INVOKE_ORDERED_SETTABLE_MAP(map)->as_ordered_map(map, out);
}

static inline int
as_settable_map(struct butterflyfish_ordered_mutable_map_i_i *const object,
                struct butterflyfish_settable_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_settable_map_i_i *map;
    seagrass_required_true(!as_ordered_settable_map(object, &map));
    return INVOKE_ORDERED_SETTABLE_MAP(map)->as_settable_map(map, out);
}

static inline int
as_incremental_map(struct butterflyfish_ordered_mutable_map_i_i *const object,
                   struct butterflyfish_incremental_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_incremental_map_i_i *map;
    seagrass_required_true(!as_ordered_incremental_map(object, &map));
    return INVOKE_ORDERED_INCREMENTAL_MAP(map)->as_incremental_map(map, out);
}

static inline int
as_reducible_map(struct butterflyfish_ordered_mutable_map_i_i *const object,
                 struct butterflyfish_reducible_map_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_reducible_map_i_i *map;
    seagrass_required_true(!as_ordered_reducible_map(object, &map));
    return INVOKE_ORDERED_REDUCIBLE_MAP(map)->as_reducible_map(map, out);
}

static inline int
as_map(const struct butterflyfish_ordered_mutable_map_i_i *const object,
       const struct butterflyfish_map_i_i **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_ordered_map_i_i *map;
    seagrass_required_true(!as_ordered_map(object, &map));
    return INVOKE_ORDERED_MAP(map)->as_map(map, out);
}

static inline int
as_insertable(struct butterflyfish_ordered_mutable_map_i_i *const object,
              struct butterflyfish_insertable_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_incremental_map_i_i *map;
    seagrass_required_true(!as_ordered_incremental_map(object, &map));
    return INVOKE_ORDERED_INCREMENTAL_MAP(map)->as_insertable(map, out);
}

static inline int
as_ordered(const struct butterflyfish_ordered_mutable_map_i_i *const object,
           const struct butterflyfish_ordered_i_i **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_ordered_map_i_i *map;
    seagrass_required_true(!as_ordered_map(object, &map));
    return INVOKE_ORDERED_MAP(map)->as_ordered(map, out);
}

static inline int
as_addable(struct butterflyfish_ordered_mutable_map_i_i *const object,
           struct butterflyfish_addable_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_incremental_map_i_i *map;
    seagrass_required_true(!as_incremental_map(object, &map));
    return INVOKE_INCREMENTAL_MAP(map)->as_addable(map, out);
}

static inline int
as_removable(struct butterflyfish_ordered_mutable_map_i_i *const object,
             struct butterflyfish_removable_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_reducible_map_i_i *map;
    seagrass_required_true(!as_reducible_map(object, &map));
    return INVOKE_REDUCIBLE_MAP(map)->as_removable(map, out);
}

static inline int
as_settable(struct butterflyfish_ordered_mutable_map_i_i *const object,
            struct butterflyfish_settable_i_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_settable_map_i_i *map;
    seagrass_required_true(!as_settable_map(object, &map));
    return INVOKE_SETTABLE_MAP(map)->as_settable(map, out);
}

static inline int
as_set(const struct butterflyfish_ordered_mutable_map_i_i *const object,
       const struct butterflyfish_set_i_i **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->as_set(map, out);
}

static inline int
as_collection(const struct butterflyfish_ordered_mutable_map_i_i *const object,
              const struct butterflyfish_collection_i_i **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_set_i_i *set;
    seagrass_required_true(!as_set(object, &set));
    return INVOKE_SET(set)->as_collection(set, out);
}

static inline int
as_stream(const struct butterflyfish_ordered_mutable_map_i_i *const object,
          const struct butterflyfish_stream_i_i **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_collection_i_i *collection;
    seagrass_required_true(!as_collection(object, &collection));
    return INVOKE_COLLECTION(collection)->as_stream(collection, out);
}

#pragma mark stream_i_i -

int butterflyfish_ordered_mutable_map_i_i_first(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_stream_i_i *stream;
    seagrass_required_true(!as_stream(object, &stream));
    return INVOKE_STREAM(stream)->first(stream, out);
}

int butterflyfish_ordered_mutable_map_i_i_next(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const item,
        const struct butterflyfish_map_i_i_entry **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!item) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_stream_i_i *stream;
    seagrass_required_true(!as_stream(object, &stream));
    return INVOKE_STREAM(stream)->next(stream, item, out);
}

#pragma mark collection_i_i -

int butterflyfish_ordered_mutable_map_i_i_count(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        uintmax_t *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_collection_i_i *collection;
    seagrass_required_true(!as_collection(object, &collection));
    return INVOKE_COLLECTION(collection)->count(collection, out);
}

int butterflyfish_ordered_mutable_map_i_i_last(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_collection_i_i *collection;
    seagrass_required_true(!as_collection(object, &collection));
    return INVOKE_COLLECTION(collection)->last(collection, out);
}

int butterflyfish_ordered_mutable_map_i_i_prev(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const item,
        const struct butterflyfish_map_i_i_entry **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!item) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_collection_i_i *collection;
    seagrass_required_true(!as_collection(object, &collection));
    return INVOKE_COLLECTION(collection)->prev(collection, item, out);
}

#pragma mark set_i_i -

int butterflyfish_ordered_mutable_map_i_i_contains(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const entry,
        bool *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!entry) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ENTRY_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_set_i_i *set;
    seagrass_required_true(!as_set(object, &set));
    return INVOKE_SET(set)->contains(set, entry, out);
}

int butterflyfish_ordered_mutable_map_i_i_contains_all(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i_i *const other,
        bool *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_set_i_i *set;
    seagrass_required_true(!as_set(object, &set));
    return INVOKE_SET(set)->contains_all(set, other, out);
}

int butterflyfish_ordered_mutable_map_i_i_get(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const entry,
        const struct butterflyfish_map_i_i_entry **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!entry) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ENTRY_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_set_i_i *set;
    seagrass_required_true(!as_set(object, &set));
    return INVOKE_SET(set)->get(set, entry, out);
}

#pragma mark addable_i_i -

int butterflyfish_ordered_mutable_map_i_i_add(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const entry) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!entry) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ENTRY_IS_NULL;
    }
    struct butterflyfish_addable_i_i *addable;
    seagrass_required_true(!as_addable(object, &addable));
    return INVOKE_ADDABLE(addable)->add(addable, entry);
}

int butterflyfish_ordered_mutable_map_i_i_add_all(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i_i *const other) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_addable_i_i *addable;
    seagrass_required_true(!as_addable(object, &addable));
    return INVOKE_ADDABLE(addable)->add_all(addable, other);
}

#pragma mark removable_i_i -

int butterflyfish_ordered_mutable_map_i_i_remove_entry(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const entry) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!entry) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ENTRY_IS_NULL;
    }
    struct butterflyfish_removable_i_i *removable;
    seagrass_required_true(!as_removable(object, &removable));
    return INVOKE_REMOVABLE(removable)->remove_item(removable, entry);
}

int butterflyfish_ordered_mutable_map_i_i_remove_all_entries(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i_i *const other) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_removable_i_i *removable;
    seagrass_required_true(!as_removable(object, &removable));
    return INVOKE_REMOVABLE(removable)->remove_all_items(removable, other);
}

#pragma mark map_i_i -

int butterflyfish_ordered_mutable_map_i_i_contains_key(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key,
        bool *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->contains_key(map, key, out);
}

int butterflyfish_ordered_mutable_map_i_i_contains_value(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const value,
        bool *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!value) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_VALUE_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->contains_value(map, value, out);
}

int butterflyfish_ordered_mutable_map_i_i_contains_all_keys(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i *const other,
        bool *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->contains_all_keys(map, other, out);
}

int butterflyfish_ordered_mutable_map_i_i_contains_all_values(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i *const other,
        bool *const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->contains_all_values(map, other, out);
}

int butterflyfish_ordered_mutable_map_i_i_get_value(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key,
        const struct sea_turtle_integer **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->get_value(map, key, out);
}

int butterflyfish_ordered_mutable_map_i_i_get_entry(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key,
        const struct butterflyfish_map_i_i_entry **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    const struct butterflyfish_map_i_i *map;
    seagrass_required_true(!as_map(object, &map));
    return INVOKE_MAP(map)->get_entry(map, key, out);
}

#pragma mark settable_map_i_i -

int butterflyfish_ordered_mutable_map_i_i_set_value(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key,
        const struct sea_turtle_integer *const value) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    if (!value) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_VALUE_IS_NULL;
    }
    struct butterflyfish_settable_map_i_i *settable_map;
    seagrass_required_true(!as_settable_map(object, &settable_map));
    return INVOKE_SETTABLE_MAP(settable_map)
            ->set_value(settable_map, key, value);
}

int butterflyfish_ordered_mutable_map_i_i_set_key(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key,
        const struct sea_turtle_integer *const value) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    if (!value) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_VALUE_IS_NULL;
    }
    struct butterflyfish_settable_map_i_i *settable_map;
    seagrass_required_true(!as_settable_map(object, &settable_map));
    return INVOKE_SETTABLE_MAP(settable_map)
            ->set_key(settable_map, key, value);
}

int butterflyfish_ordered_mutable_map_i_i_set_entry(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const entry,
        const struct butterflyfish_map_i_i_entry *const value) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!entry) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ENTRY_IS_NULL;
    }
    if (!value) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_VALUE_IS_NULL;
    }
    struct butterflyfish_settable_map_i_i *settable_map;
    seagrass_required_true(!as_settable_map(object, &settable_map));
    return INVOKE_SETTABLE_MAP(settable_map)
            ->set_entry(settable_map, entry, value);
}

#pragma mark incremental_map_i_i -

int butterflyfish_ordered_mutable_map_i_i_add_value(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key,
        const struct sea_turtle_integer *const value) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    if (!value) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_VALUE_IS_NULL;
    }
    struct butterflyfish_incremental_map_i_i *incremental_map;
    seagrass_required_true(!as_incremental_map(object, &incremental_map));
    return INVOKE_INCREMENTAL_MAP(incremental_map)
        ->add_value(incremental_map, key, value);
}

#pragma mark insertable_i_i -

int butterflyfish_ordered_mutable_map_i_i_insert(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const item,
        const struct butterflyfish_map_i_i_entry *const value) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!item) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ITEM_IS_NULL;
    }
    if (!value) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_VALUE_IS_NULL;
    }
    struct butterflyfish_insertable_i_i *insertable;
    seagrass_required_true(!as_insertable(object, &insertable));
    return INVOKE_INSERTABLE(insertable)->insert(insertable, item, value);
}

int butterflyfish_ordered_mutable_map_i_i_insert_all(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i_entry *const item,
        const struct butterflyfish_stream_i_i *const other) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!item) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_ITEM_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_insertable_i_i *insertable;
    seagrass_required_true(!as_insertable(object, &insertable));
    return INVOKE_INSERTABLE(insertable)->insert_all(insertable, item, other);
}

#pragma mark reducible_map_i_i -

int butterflyfish_ordered_mutable_map_i_i_remove(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct sea_turtle_integer *const key) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!key) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_KEY_IS_NULL;
    }
    struct butterflyfish_reducible_map_i_i *reducible_map;
    seagrass_required_true(!as_reducible_map(object, &reducible_map));
    return INVOKE_REDUCIBLE_MAP(reducible_map)->remove(reducible_map, key);
}

int butterflyfish_ordered_mutable_map_i_i_remove_all(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i *const other) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_reducible_map_i_i *reducible_map;
    seagrass_required_true(!as_reducible_map(object, &reducible_map));
    return INVOKE_REDUCIBLE_MAP(reducible_map)
        ->remove_all(reducible_map, other);
}

int butterflyfish_ordered_mutable_map_i_i_retain_all(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i *const other) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!other) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_reducible_map_i_i *reducible_map;
    seagrass_required_true(!as_reducible_map(object, &reducible_map));
    return INVOKE_REDUCIBLE_MAP(reducible_map)
        ->retain_all(reducible_map, other);
}

#pragma mark ordered_mutable_map_i_i -

int butterflyfish_ordered_mutable_map_i_i_as_stream(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_stream_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_stream(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_collection(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_collection_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_collection(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_set(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_set_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_set(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_map(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_settable(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_settable_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_settable(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_addable(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_addable_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_addable(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_removable(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_removable_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_removable(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_ordered_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_insertable(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_insertable_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_insertable(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_settable_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_settable_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_settable_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_incremental_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_incremental_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_reducible_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_reducible_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered_map(
        const struct butterflyfish_ordered_mutable_map_i_i *const object,
        const struct butterflyfish_ordered_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_settable_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_settable_incremental_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_settable_incremental_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_settable_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_settable_reducible_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_settable_reducible_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_incremental_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered_incremental_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered_settable_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered_settable_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_reducible_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered_reducible_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered_settable_incremental_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_incremental_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered_settable_incremental_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_ordered_settable_reducible_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_reducible_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_ordered_settable_reducible_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_as_mutable_map(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_mutable_map_i_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return as_mutable_map(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_keys(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_mutable_set_i **const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return INVOKE(object)->keys(object, out);
}

int butterflyfish_ordered_mutable_map_i_i_values(
        struct butterflyfish_ordered_mutable_map_i_i *const object,
        struct butterflyfish_ordered_settable_reducible_list_i **const
                out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_MUTABLE_MAP_I_I_ERROR_OUT_IS_NULL;
    }
    return INVOKE(object)->values(object, out);
}
