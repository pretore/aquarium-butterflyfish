...
#define INVOKABLE   (*(struct butterflyfish_sorted_set_ni **) object)
...
int butterflyfish_sorted_set_ni_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_SORTED_SET_NI_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_SORTED_SET_NI_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_sorted_set_ni_count(
    ...
    return INVOKABLE->ordered_set_ni
            .set_ni
            .collection_ni
            .count(object, out);
}
...
int butterflyfish_sorted_set_ni_add_all(
    ...
    if (!other) {
        return BUTTERFLYFISH_SORTED_SET_NI_ERROR_OTHER_IS_NULL;
    }
    return INVOKABLE->ordered_set_ni
            .set_ni
