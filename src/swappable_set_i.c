...
#define INVOKE(x)               (*(struct butterflyfish_swappable_set_i **) x)
...
#define INVOKED_ORDERED(x)      (*(struct butterflyfish_ordered_i **) x)
#define INVOKE_COLLECTION(x)    (*(struct butterflyfish_collection_i **) x)
#define INVOKE_STREAM(x)        (*(struct butterflyfish_stream_i **) x)

static inline int
as_swappable(struct butterflyfish_swappable_set_i *const object,
             struct butterflyfish_swappable_i **const out) {
    assert(object);
    assert(out);
    return INVOKE(object)->as_swappable(object, out);
}

static inline int
as_ordered_set(const struct butterflyfish_swappable_set_i *const object,
...
static inline int
as_set(const struct butterflyfish_swappable_set_i *const object,
...
static inline int
as_ordered(const struct butterflyfish_swappable_set_i *const object,
...
static inline int
as_collection(const struct butterflyfish_swappable_set_i *const object,
...
static inline int
as_stream(const struct butterflyfish_swappable_set_i *const object,
...
int butterflyfish_swappable_set_i_first(
    ...
    if (!object) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_swappable_set_i_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_swappable_set_i_swap(
    ...
    if (!other) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_OTHER_IS_NULL;
    }
...
int butterflyfish_swappable_set_i_contains(
    ...
    if (!value) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_VALUE_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_SWAPPABLE_SET_I_ERROR_OUT_IS_NULL;
    }
