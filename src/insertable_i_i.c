...
#define INVOKE(x)               (*(struct butterflyfish_insertable_i_i **) x)
...
static inline int
as_ordered(const struct butterflyfish_insertable_i_i *const object,
...
static inline int
as_collection(const struct butterflyfish_insertable_i_i *const object,
              const struct butterflyfish_collection_i_i **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_ordered_i_i *ordered;
    seagrass_required_true(!as_ordered(object, &ordered));
    return INVOKE_ORDERED(ordered)->as_collection(ordered, out);
}

static inline int
as_stream(const struct butterflyfish_insertable_i_i *const object,
