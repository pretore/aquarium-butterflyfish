...
as_ordered_map(
    ...
    assert(object);
    assert(out);
    struct butterflyfish_ordered_incremental_map_i_i *ordered_incremental_map;
...
as_settable_map(
    ...
    assert(object);
    assert(out);
    struct butterflyfish_settable_incremental_map_i_i *ordered_incremental_map;
...
static inline int
as_addable(
        struct butterflyfish_ordered_settable_incremental_map_i_i *const
                object,
...
static inline int
as_map(const struct butterflyfish_ordered_settable_incremental_map_i_i
            *const object,
...
static inline int
as_set(const struct butterflyfish_ordered_settable_incremental_map_i_i
            *const object,
...
static inline int
as_stream(const struct butterflyfish_ordered_settable_incremental_map_i_i
            *const object,
...
int butterflyfish_ordered_settable_incremental_map_i_i_count(
        const struct butterflyfish_ordered_settable_incremental_map_i_i *const object,
        uintmax_t *const out) {
    if (!object) {
        return
        BUTTERFLYFISH_ORDERED_SETTABLE_INCREMENTAL_MAP_I_I_ERROR_OBJECT_IS_NULL;
