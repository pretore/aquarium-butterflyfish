...
as_sorted_reducible_list(
    ...
    assert(object);
    assert(out);
    return INVOKE(object)->as_sorted_reducible_list(object, out);
}
...
static inline int
as_list(const struct butterflyfish_sorted_settable_reducible_list_i
                *const object,
...
static inline int
as_sorted(const struct butterflyfish_sorted_settable_reducible_list_i
                *const object,
...
int butterflyfish_sorted_settable_reducible_list_i_remove_all_items(
    ...
    if (!other) {
        return
        BUTTERFLYFISH_SORTED_SETTABLE_REDUCIBLE_LIST_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_removable_i *removable;
    seagrass_required_true(!as_removable(object, &removable));
...
#pragma mark sorted_settable_reducible_list_i -
