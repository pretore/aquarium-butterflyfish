...
static inline int
as_ordered(const struct butterflyfish_ordered_reducible_list_i *const object,
...
static inline int
as_removable(struct butterflyfish_ordered_reducible_list_i *const object,
             struct butterflyfish_removable_i **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_reducible_list_i *reducible;
...
static inline int
as_list(const struct butterflyfish_ordered_reducible_list_i *const object,
...
static inline int
as_collection(const struct butterflyfish_ordered_reducible_list_i *const object,
...
static inline int
as_stream(const struct butterflyfish_ordered_reducible_list_i *const object,
...
int butterflyfish_ordered_reducible_list_i_first(
    ...
    if (!object) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_I_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_I_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_ordered_reducible_list_i_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_I_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_I_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_ordered_reducible_list_i_remove_all_items(
    ...
    if (!other) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_I_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_removable_i *removable;
    seagrass_required_true(!as_removable(object, &removable));
...
#pragma mark ordered_reducible_list_i -
