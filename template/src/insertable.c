...
#define INVOKE(x)               (*(struct butterflyfish_insertable_$t$ **) x)
...
static inline int
as_ordered(const struct butterflyfish_insertable_$t$ *const object,
...
static inline int
as_collection(const struct butterflyfish_insertable_$t$ *const object,
              const struct butterflyfish_collection_$t$ **const out) {
    assert(object);
    assert(out);
    const struct butterflyfish_ordered_$t$ *ordered;
    seagrass_required_true(!as_ordered(object, &ordered));
    return INVOKE_ORDERED(ordered)->as_collection(ordered, out);
}

static inline int
as_stream(const struct butterflyfish_insertable_$t$ *const object,
...
int butterflyfish_insertable_$t$_first(
    ...
    if (!out) {
        return BUTTERFLYFISH_INSERTABLE_$T$_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_insertable_$t$_insert_all(
    ...
    if (!other) {
        return BUTTERFLYFISH_INSERTABLE_$T$_ERROR_OTHER_IS_NULL;
    }
