...
#define INVOKE(x)  \\
    (*(struct butterflyfish_ordered_settable_incremental_map_$t$ **) x)
...
#define INVOKE_ORDERED_INCREMENTAL_MAP(x) \\
    (*(struct butterflyfish_ordered_incremental_map_$t$ **) x)
...
as_ordered_incremental_map(
    ...
    assert(object);
    assert(out);
    return INVOKE(object)->as_ordered_incremental_map(object, out);
}
...
as_ordered_map(
    ...
    assert(object);
    assert(out);
    struct butterflyfish_ordered_incremental_map_$t$ *ordered_incremental_map;
    seagrass_required_true(!as_ordered_incremental_map(
            (void *) object, &ordered_incremental_map));
    return INVOKE_ORDERED_INCREMENTAL_MAP(ordered_incremental_map)
        ->as_ordered_map(ordered_incremental_map, out);
}
...
as_settable_map(
    ...
    assert(object);
    assert(out);
    struct butterflyfish_settable_incremental_map_$t$ *ordered_incremental_map;
    seagrass_required_true(!as_settable_incremental_map(
            object, &ordered_incremental_map));
    return INVOKE_SETTABLE_INCREMENTAL_MAP(ordered_incremental_map)
        ->as_settable_map(ordered_incremental_map, out);
}
...
as_incremental_map(
    ...
    seagrass_required_true(!as_settable_incremental_map(
            object, &ordered_incremental_map));
    return INVOKE_SETTABLE_INCREMENTAL_MAP(ordered_incremental_map)
        ->as_incremental_map(ordered_incremental_map, out);
}
...
static inline int
as_addable(
        struct butterflyfish_ordered_settable_incremental_map_$t$ *const
                object,
...
static inline int
as_insertable(struct butterflyfish_ordered_settable_incremental_map_$t$
                *const object,
              struct butterflyfish_insertable_$t$ **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_ordered_incremental_map_$t$ *map;
...
static inline int
as_map(const struct butterflyfish_ordered_settable_incremental_map_$t$
            *const object,
...
static inline int
as_set(const struct butterflyfish_ordered_settable_incremental_map_$t$
            *const object,
...
static inline int
as_stream(const struct butterflyfish_ordered_settable_incremental_map_$t$
            *const object,
...
int butterflyfish_ordered_settable_incremental_map_$t$_count(
        const struct butterflyfish_ordered_settable_incremental_map_$t$ *const object,
        uintmax_t *const out) {
    if (!object) {
        return
        BUTTERFLYFISH_ORDERED_SETTABLE_INCREMENTAL_MAP_$T$_ERROR_OBJECT_IS_NULL;
...
#pragma mark ordered_settable_incremental_map_$t$ -
...
butterflyfish_ordered_settable_incremental_map_$t$_as_ordered_incremental_map(
    ...
    return as_ordered_incremental_map(object, out);
}
