...
#define INVOKE(x)               (*(struct butterflyfish_settable_$t$ **) x)
...
static inline int
as_stream(const struct butterflyfish_settable_$t$ *const object,
...
int butterflyfish_settable_$t$_first(
    ...
    if (!out) {
        return BUTTERFLYFISH_SETTABLE_$T$_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_settable_$t$_set_item(
        struct butterflyfish_settable_$t$ *const object,
        const $v$const item,
        const $v$const value) {
    if (!object) {
        return BUTTERFLYFISH_SETTABLE_$T$_ERROR_OBJECT_IS_NULL;
