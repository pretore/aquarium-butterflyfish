...
static inline int
as_ordered(const struct butterflyfish_ordered_reducible_list_$t$ *const object,
...
static inline int
as_removable(struct butterflyfish_ordered_reducible_list_$t$ *const object,
             struct butterflyfish_removable_$t$ **const out) {
    assert(object);
    assert(out);
    struct butterflyfish_reducible_list_$t$ *reducible;
...
static inline int
as_list(const struct butterflyfish_ordered_reducible_list_$t$ *const object,
...
static inline int
as_collection(const struct butterflyfish_ordered_reducible_list_$t$ *const object,
...
static inline int
as_stream(const struct butterflyfish_ordered_reducible_list_$t$ *const object,
...
int butterflyfish_ordered_reducible_list_$t$_first(
        ...
        const $v$*const out) {
    if (!object) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_$T$_ERROR_OBJECT_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_$T$_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_ordered_reducible_list_$t$_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_$T$_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_$T$_ERROR_OUT_IS_NULL;
    }
...
int butterflyfish_ordered_reducible_list_$t$_remove_all_items(
    ...
    if (!other) {
        return BUTTERFLYFISH_ORDERED_REDUCIBLE_LIST_$T$_ERROR_OTHER_IS_NULL;
    }
    struct butterflyfish_removable_$t$ *removable;
    seagrass_required_true(!as_removable(object, &removable));
...
#pragma mark ordered_reducible_list_$t$ -
