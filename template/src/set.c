...
static inline int
as_stream(const struct butterflyfish_set_$t$ *const object,
...
int butterflyfish_set_$t$_first(
        const struct butterflyfish_set_$t$ *const object,
        const $v$*const out) {
    if (!object) {
...
int butterflyfish_set_$t$_next(
    ...
    if (!item) {
        return BUTTERFLYFISH_SET_$T$_ERROR_ITEM_IS_NULL;
    }
    if (!out) {
        return BUTTERFLYFISH_SET_$T$_ERROR_OUT_IS_NULL;
    }
